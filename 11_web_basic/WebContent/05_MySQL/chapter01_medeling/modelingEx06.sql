...
# 4) AUTO_INCREMENT : 인덱스 자동증가 ( PRIMARY KEY나 UNIQUE 속성과 같이 사용한다. )
CREATE TABLE TEST4 (
...
INSERT INTO TEST4 VALUES(1, '상품1', 1000 , NOW());				# SUCCESS - 1
INSERT INTO TEST4(PRODUCT_NM) VALUES ('상품2');					# SUCCESS - 2
INSERT INTO TEST4(PRODUCT_NM) VALUES ('상품3');					# SUCCESS - 3

INSERT INTO TEST4 VALUES(10, '상품4', 4000 , '2022-12-31');		# SUCCESS - 10
INSERT INTO TEST4(PRODUCT_NM) VALUES ('상품5');					# SUCCESS - 11 ( 마지막 CD 값 이후부터 1 증가 )
...
CREATE TABLE TEST5 (
	PRODUCT_CD INT, 		# PRIMARY KEY, (방법 1)
    PRODUCT_NM VARCHAR(20),
    PRICE	   INT,
    REG_DT     DATETIME,
    PRIMARY KEY (PRODUCT_CD) # (방법 2)
);
...
UPDATE TEST6_1 SET PRODUCT_CD = 11 WHERE PRODUCT_CD = 1;	# 메인테이블과 참조테이블사이의 데이터의 차이가 발생
UPDATE TEST6_1 SET PRODUCT_CD = 22 WHERE PRODUCT_CD = 2;	# 메인테이블과 참조테이블사이의 데이터의 차이가 발생
DELETE FROM TEST6_1 WHERE PRODUCT_CD = 3;					# 메인테이블과 참조테이블사이의 데이터의 차이가 발생
...
CREATE TABLE TEST7_2(
    ...
    MEMBER_ID VARCHAR(20),
    PRODUCT_CD INT,
    # FOREIGN KEY (PRODUCT_CD) REFERENCES TEST7_1(PRODUCT_CD) 							# 참조키 설정 ( 방법 1 )
    CONSTRAINT PRODUCT_CD_REF1 FOREIGN KEY (PRODUCT_CD) REFERENCES TEST7_1(PRODUCT_CD)	# 참조키 설정 ( 방법 2 )
);
...
INSERT INTO TEST7_2 VALUES ('O6' , '유저1' , 4); 				# ERROR - KEY 참조 제약 위반 (메인 키에 없는 데이터)
INSERT INTO TEST7_2 VALUES ('O7' , '유저2' , 5); 				# ERROR - KEY 참조 제약 위반 (메인 키에 없는 데이터)

UPDATE TEST7_1 SET PRODUCT_CD = 11 WHERE PRODUCT_CD = 1;	# 참조키가 있을 경우 메인키의 데이터 수정 불가
UPDATE TEST7_1 SET PRODUCT_CD = 22 WHERE PRODUCT_CD = 2;	# 참조키가 있을 경우 메인키의 데이터 수정 불가
DELETE FROM TEST7_1 WHERE PRODUCT_CD = 3;					# 참조키가 있을 경우 메인키의 데이터 삭제 불가
...
CREATE TABLE TEST8_2(
    ...
    PRODUCT_CD INT,
    # FOREIGN KEY (PRODUCT_CD) REFERENCES TEST7_1(PRODUCT_CD) 							# 참조키 설정 ( 방법 1 )
    CONSTRAINT PRODUCT_CD_REF2 FOREIGN KEY (PRODUCT_CD) REFERENCES TEST8_1(PRODUCT_CD)	# 참조키 설정 ( 방법 2 )
...
INSERT INTO TEST8_2 VALUES ('O6' , '유저1' , 4); 				# ERROR - KEY 참조 제약 위반 (메인 키에 없는 데이터)
INSERT INTO TEST8_2 VALUES ('O7' , '유저2' , 5); 				# ERROR - KEY 참조 제약 위반 (메인 키에 없는 데이터)

UPDATE TEST8_1 SET PRODUCT_CD = 11 WHERE PRODUCT_CD = 1;	# 메인키의 데이터가 수정될 경우 참조키의 데이터도 같이 수정
UPDATE TEST8_1 SET PRODUCT_CD = 22 WHERE PRODUCT_CD = 2;	# 메인키의 데이터가 수정될 경우 참조키의 데이터도 같이 수정
DELETE FROM TEST8_1 WHERE PRODUCT_CD = 3;					# 메인키의 데이터가 삭제될 경우 참조키의 데이터는 NULL로 수정
