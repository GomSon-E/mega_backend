...
SELECT
		SUM(O.ORDER_GOODS_QTY) AS TOTAL_ORDER_QTY
FROM 
		  ORDER_TB O
        INNER JOIN MEMBER_TB M
				ON O.MEMBER_ID = M.MEMBER_ID
...
SELECT
		SUM(ORDER_GOODS_QTY) AS TOTAL_ORDER_QTY,
        M.RESIDENCE
FROM 
		ORDER_TB O
        JOIN MEMBER_TB M
...
FROM
        ...
        ON O.PRODUCT_CD = P.PRODUCT_CD
WHERE
		PRODUCT_NM = 'Apple 2020 맥북 에어 13';
...
FROM
        ...
        JOIN PRODUCT_TB P
        ON O.PRODUCT_CD = P.PRODUCT_CD
		AND ORDER_DT BETWEEN '2020-01-01' AND '2020-12-31';
