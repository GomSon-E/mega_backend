...
/*
    ...
    - 서브쿼리 사용시 주의사항
		...
		3) 서브쿼리에서는 ORDER BY를 사용하지 못한다. -> 우회하는 방식을 사용해야 함
...
FROM CAR C
	 ...
	 AND R.RENT_PERIOD IN (SELECT MAX(RENT_PERIOD) 
						   FROM RENT);

        
# 현대 자동차를 주문한 회원정보 조회하기. ( JOIN 2개로 가능 )
SELECT *
FROM MEMBER
WHERE MEMBER_ID IN (SELECT MEMBER_ID
					FROM RENT
...
#(참고)
# 자동차테이블의 백업 테이블을 생성하고 데이터를 이동하기.
...
# 가장 비싼 차량3개의 가격을 10%할인하기 (테이블명의 중첩될 경우 (테이블을 임의적으로 만들 경우) AS로 테이블명 지정)
UPDATE CAR
SET PRICE = PRICE - PRICE * 0.1
WHERE CAR_CD IN (SELECT T1.CAR_CD
				 FROM (SELECT *
					   FROM CAR 
					   ORDER BY PRICE DESC 
					   LIMIT 3) AS T1);
...
# 'k7' 차량의 렌트가격에서 전체 차량의 평균 렌트가격을 뺀 가격을 조회하기
# SELECT PRICE - AVG(PRICE) FROM CAR WHERE CAR_NM = 'k7';	# ERROR
...
SELECT CAR_NM, PRICE, PRICE - (SELECT AVG(PRICE) FROM CAR)
FROM CAR;


# 3) 인라인 뷰 예시 = 가상의 테이블 만드기 -> 반드시 이름이 있어야 함 (AS 생략 가능)

# 렌트카들의 평균 가격보다 높은 차량들의 정보를 조회하기. ( 중첩 서브 쿼리로도 가능 )
SELECT AVG(PRICE) FROM CAR;
