...
# 상품코드가 'P10004'이상인 상품 전체 컬럼 조회 ( 숫자를 문자로 취급해서 비교 ex) 9 > 12 )		
...
# 상품의 전체 컬럼 검색 (가격이 높은 순서대로 정렬 , 최근에 등록된 상품순서로 정렬)
...
# 상품의 전체 컬럼 검색 (가격이 낮은 순서대로 정렬 , 최근에 등록된 상품순서로 정렬)
...
# 상품의 전체 컬럼 검색 (최근에 등록된 상품순서로 정렬 , 가격이 높은 순으로 정렬)
...
/*
    ...
    [ 형식 ]
	LIMIT x 	: x row만 조회
	LIMIT x,y   : x번째 부터 y row만 조회 (0번째 부터 시작)
