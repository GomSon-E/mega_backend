 ...
 * 
 * 	- 변수명은 영어로 작성한다. ( 한글x ) 
 *  - 변수명은 소문자로 시작한다. ( 클래스이름은 대문자로 시작한다. )
 *  - 두 단어가 이어지는 경우 이어지는 단어의 첫글자를 대문자로 작성한다. (camel case) -> 화면
 *    Ex) productCode , productName
 *  - 두 단어가 이어지는 경우 이어지는 단어를 _로 이어준다. (snake case) -> DB
 *    Ex) product_code , product_name
 *  
 *  - 상수명은 전부 대문자로 작성한다. (나중에 설명)
 *  - 상수명에서 두 단어가 이어지는 경우 _로 이어준다.
 *  
 *  - 불리언 변수명 앞에는 is를 붙인ㅊ 다. 
