...
public class Advice {
	...
	public void aroundGetWorkingTime(ProceedingJoinPoint pjp) {
		...
		// ProceedingJoinPoint의  proceed(); 메서드를 사용하여 타겟팅 메서드를 실행한다.
		try {
			pjp.proceed();
		} catch (Throwable e) {
			e.printStackTrace();
		...
		System.out.println("메서드 실행 시간 : " + (endTime - startTime) + "초");
		System.out.println("===============\n");
	...
	public void AfterReturningGetInfo(JoinPoint jp) { 				// JoinPoint를 통하여 메서드의 파라메타를 전달받을 수 있다.
		...
		System.out.println("3 : " + jp.getSignature().getName());	// 어드바이즈메서드에 대한 설명(description)이 반환
