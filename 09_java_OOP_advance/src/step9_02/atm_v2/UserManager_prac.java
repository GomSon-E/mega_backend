...
public class UserManager_prac {
	...
	private UserManager_prac() {}
	static private UserManager_prac instance = new UserManager_prac();
	static public UserManager_prac getInstance() {
		return instance;
	}
	...
	int logUser() {
		...
		for (int i = 0; i < UserManager_prac.instance.userCnt; i++) {
	...
	void joinMember() {
		...
		if (userCnt == 0) {
			userList = new User_prac[userCnt + 1];
			userList[userCnt] = new User_prac();
		}
		else {
			User_prac[] tmp = userList;
			userList = new User_prac[userCnt + 1];
			userList[userCnt] = new User_prac();
		...
		FileManager_prac.getInstance().save();
	...
	void setDummy() {
		...
		userCnt = 5;
		userList = new User_prac[userCnt];
		
		for (int i = 0; i < userCnt; i++) {
			userList[i] = new User_prac();
			userList[i].id = ids[i];
			userList[i].pw = pws[i];
			userList[i].acc[0] = new Account_prac();
