...
<body>
	...
	<%
		...
		BoardAdvanceDao bdao = BoardAdvanceDao.getInstance();
		int totalBoardCount = bdao.getAllCount(searchKeyword,searchWord);
		
		
		int startBoardIdx = (currentPageNumber -1) * onePageViewCount;
		ArrayList<BoardAdvanceDto> boardList = bdao.getSearchBoard(searchKeyword, searchWord, startBoardIdx, onePageViewCount);
	%>
	...
	<%
		for (int i=0; i<boardList.size(); i++){
			BoardAdvanceDto boardDto = boardList.get(i);
	%>						
			<tr align="center">
				<td><%= boardDto.getNum() %></td>
				<td align="left">
					<%
						if (boardDto.getReStep() > 1) {
							for (int j=0; j<(boardDto.getReLevel()-1)*5; j++) {
					%>
								&nbsp;
					<% 			
							}
					...
					%>
					 <a href="05_bInfo.jsp?num=<%=boardDto.getNum()%>"><%= boardDto.getSubject() %></a>
				</td>
